...
module.exports = class DiscordBot {
            ...
            switch (messageSplit[0]) {
                ...
                case 'random':
                    if (messageSplit.length === 3) {
                        ...
                        this.music.playAtTop(message.guild, message.channel, message.member.voice.channel, this.music.getRandomOzjasz(), serverQueue);
                    break;
                case 'brzechwa':
                    if (messageSplit.length === 3) {
                        const voiceChannel = Utils.getUserVoiceChannel(message, Utils.getUserId(messageSplit[2]));
    ...
    deleteScheduleCommand(message){
        ...
        if(!this.scheduledJobs.get(guildID).has(jobID))
            Utils.shortEmbedReply(message.channel, 'Nie ma takiego utworu!');
        else {
