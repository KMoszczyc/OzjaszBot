...
module.exports = class Utils {
    ...
    static async sendEmbeds(title, text, message) {
        const message_embed = new Discord.MessageEmbed().setColor(0xa62019).setTitle(title);
        let prevIndex = 0;
    ...
    static async getRandomLineWithText(filename, text) {
        let data = await Utils.readFile(filename);
        const lines = data.split("\n");
    ...
    static convertIsoTime(isoTime) {
        ...
        if (hourIndex !== -1) convertedTime += isoTime.substring(0, hourIndex);

        if (minIndex !== -1) {
            if (hourIndex !== -1) convertedTime += ":";
        ...
        if (secIndex !== -1) {
            ...
            if (secondInt < 10) convertedTime += "0";
    ...
    static parseHour(hour) {
        ...
        if (isNaN(hourInt) || hourInt < 0 || hourInt > 23) throw new Error("Wrong hour!");
        return hourInt;
    }

    static parseMinute(minute) {
        let minuteInt = parseInt(minute);
        if (isNaN(minuteInt) || minuteInt < 0 || minuteInt > 60) throw new Error("Wrong minute!");
        return minuteInt;
    }
